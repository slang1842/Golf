class SingleFieldStatistic < ActiveRecord::Base
	...
	def self.calculate_stats(user)
		...
		field_id_arr.uniq.each do |field_id|
			SingleFieldStatistic.check_for_existence(field_id, user, @single_field_stats, games)
		end
		return true
	end

	def self.check_for_existence(field_id, user, all_stats, games)
		stats = all_stats.detect {|s| s.user_id == user.id && s.field_id == field_id}
		if stats == nil
			stats = SingleFieldStatistic.create(:user_id => user.id, :field_id => field_id)
			stats.save!
		end
		SingleFieldStatisticsByHole.check_for_existence(stats, user, games)
